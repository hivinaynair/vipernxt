...
`/next` runs `plan` then `build`. `build` is the factory: isolate (own branch
from `staging`) → implement (feature folders; hoist repeated mechanics) →
prove (journey-ID evidence) → ship (draft PR; review bot only if installed).
Those skills are host-agnostic — they do not name a cloud or a model as a
prerequisite. `shape` and `design-system` read this file for the constraints
above.
