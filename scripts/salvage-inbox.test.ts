...
describe("salvage-inbox.mjs", () => {
  test("copies text into pages and writes inventory", () => {
    ...
    try {
      ...
      execFileSync("bun", [script, src, "--inbox", inbox], { encoding: "utf8" });
  ...
  test("keeps captions and Not-here on re-run", () => {
    const dir = mkdtempSync(join(tmpdir(), "pile-"));
    try {
      const src = join(dir, "drop");
      const inbox = join(dir, "inbox");
      ...
      execFileSync("bun", [script, src, "--inbox", inbox], { encoding: "utf8" });
      writeFileSync(
        join(inbox, "INVENTORY.md"),
        `# Salvage inbox

| Page | From | Kind | Caption |
|---|---|---|---|
| \`ledgers.csv\` | \`ledgers.csv\` | text — read directly | Last ten lets. Median 16. |

## Not here

- No photograph of the wall chart
`,
      );
      execFileSync("bun", [script, src, "--inbox", inbox], { encoding: "utf8" });
      const inv = readFileSync(join(inbox, "INVENTORY.md"), "utf8");
      expect(inv).toContain("Last ten lets. Median 16.");
      expect(inv).toContain("No photograph of the wall chart");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  test("unpacks a zip into pages", () => {
    ...
    try {
      ...
      execFileSync("bun", [script, zip, "--inbox", inbox], { encoding: "utf8" });
      expect(readFileSync(join(inbox, "pages", "ledgers.csv"), "utf8")).toContain("amount");
